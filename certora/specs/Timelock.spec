// Based on Timelock.spec in SiloVault spec

import "Enabled.spec";
using ERC20Helper as ERC20Helper;

methods {
    function ERC20Helper.balanceOf(address token, address user) external returns (uint256) envfree;
}
...
rule removableTime(env e_next, method f, calldataarg args) {
    ...
    if (e_next.block.timestamp < nextTime)  {
        ...
        // assert ERC20Helper.balanceOf(id,currentContract) => config_(id).enabled; 
        assert config_(id).balance > 0 => config_(id).enabled; //CHANGED DUE TO FIX
