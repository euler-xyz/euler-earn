...
contract DonationAttackHandler is BaseHandler {
    ...
    function simulateYieldAccrual(uint256 amount, uint8 i) external {
        address strategy = _getRandomStrategy(i);

        assetTST.mint(strategy, amount);
        IEVault(strategy).skim(type(uint256).max, address(eulerEulerEarnVault));
    }
